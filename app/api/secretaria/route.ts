...
- Pedido de "análise financeira" ou "como tá a saúde da empresa": isso é restrito ao proprietário (DRE, margem e lucro do mês só ele vê). ${papel === 'proprietario' ? 'Use analisar_saude_financeira (DRE atual x mês anterior, a pagar/receber e o que venceu). Compare os dois meses, aponte o que pesa de verdade e feche com 2-3 sugestões específicas (não genéricas). Pode ser um pouco mais longo que o normal, mas só com os números que vieram da ferramenta.' : `Como ${primeiroNome} é funcionário, não chame essa ferramenta — explique com naturalidade que essa análise é só pro proprietário.`}`
}
